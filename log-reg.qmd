...
I det følgende skal vi se på logistisk regression. Måske har du allerede hørt begrebet logistisk regression **ME: Eller *du har måske hørt begrebet \"logistisk\" * ** i gymnasieundervisningen i forbindelse med logistisk vækst. Dette er en anden type af logistisk regression end den, der behandles i denne note. I slutningen af noten vil vi dog se et eksempel, hvor der alligevel er en sammenhæng mellem logistisk regression og logistisk vækst.
...
**ME: Funktionen hedder vel $O$ og ikke $f$?**
```{r echo=FALSE}
...
**ME: Kommentar om at $O$ er voksende? Måske opgave hvor man kan vise at $O'>0$? **
...
e^y =  \frac{p}{1-p}. 
$$
**ME: Punktum efter udtryk (ved godt at det er korrekt) **
...
** ME: $b$ er sat til $-0.5$ i stedet for $-5$ nedenfor. **
```{r echo=FALSE}
a=2
b=-0.5
...
I app'en herunder ser du grafen for $f(x)=...$ 
...
Vi vender nu tilbage til vores data og lader $A_1$ være hændelsen at første patient har sygdomsstatus $y_1$, $A_2$ være hændelsen at anden patient har sygdomsstatus $y_2$ og så videre. Bemærk, at $P(A_i)$ er det samme, som det vi tidligere kaldte $p_i(a,b)$.
...
Ved at bruge (@eq-p-logistisk) i forbindelse med den første sum og efterfølgende samle leddene i en sum, fås
...
\end{align*}
** ME: Jeg ville skriv $=0$ til sidst...**
...
Her på siden har vi flere eksempler på algoritmer, som vil kunne bruges til at prædiktere om patienter er syge eller raske, fx [neurale netværk](../neurale_net/neurale_net.qmd)[^4] og [Bayes klassifikation](../naivbayes/NaivBayes.qmd) **NOTE:flere?**. 
...
[^4]: Logistisk regression er i øvrigt et meget simpelt eksempel på et neuralt netværk. ** ME: Hvis man vælge cross entropy tabsfunktionen...?**
